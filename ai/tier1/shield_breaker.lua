...
local BreakerAI = setmetatable({}, { __index = BaseAI })
robot.missiles = {}

function BreakerAI:Init(id)
    BaseAI.Init(self, id)
...
function BreakerAI:GetPriority(target)
    if IsShieldDevice(target) then
        return 10
    end
    return 1
end

function BreakerAI:FindTarget()
    if self.scanCooldown > 0 then return end
    self.scanCooldown = 0.4
...
function BreakerAI:KeepDistance()
    if not self.target then return end
...
function BreakerAI:Attack()
    if self.fireCooldown > 0 then return end
    if not self.target then return end

    FireShieldBreakerShot(self.id, self.target)
    self.fireCooldown = GetWeaponReloadTime(self.id)
end

function BreakerAI:Update(dt)
